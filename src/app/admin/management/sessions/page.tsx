...
function SessionGroup({
  ...
  onDeleteRequest: (s: FullSession) => void;
  onReordered: () => Promise<void>;
}) {
  const { list, dragPropsFor } = useDragReorder(sessions, async (orderedIds) => {
    await sendJson("/api/admin/sessions/reorder", "POST", { adventureId, ids: orderedIds });
    await onReordered();
  });

  if (sessions.length === 0) {
    return <p className="text-sm text-guild-muted">Nenhuma sessão ainda.</p>;
  }

  return (
    <ul className="space-y-2">
      {list.map((s) => (
        <li key={s.id} {...dragPropsFor(s.id)} className="panel flex cursor-move items-center gap-3 p-4">
